...
class SaleOrder(models.Model):
    ...
    def action_create_project_tasks(self):
        ...
        Görevler, belirli stage'lere bağlı kalmadan oluşturulur.
        """
        for order in self:
            ...
            for line in order.order_line:
                if line.product_id and line.product_id.type in ['product', 'consu']:
                    ...
                    combined_mo_name = line.product_id.name
                    task_description_suffix = ""

                    if related_mo:
                        if hasattr(related_mo, 'urun_adi') and related_mo.urun_adi and \
                           hasattr(related_mo, 'urun_aciklama') and related_mo.urun_aciklama:
                            combined_mo_name = f"{related_mo.urun_adi} {related_mo.urun_aciklama}"
                        elif hasattr(related_mo, 'urun_adi') and related_mo.urun_adi:
                            combined_mo_name = related_mo.urun_adi
                        elif hasattr(related_mo, 'urun_aciklama') and related_mo.urun_aciklama:
                            combined_mo_name = related_mo.urun_aciklama

                        if hasattr(related_mo, 'urun_aciklama') and related_mo.urun_aciklama:
                            task_description_suffix = f" - {related_mo.urun_aciklama}"

                    # Sadece tek bir görev oluşturuluyor
                    self.env['project.task'].create({
                        'name': f"{combined_mo_name} - Üretim Görevi", # Görev adı daha genel
                        'project_id': order.project_id.id,
                        'description': _(
                            f"Satış Siparişi #{order.name} için ürün ({line.product_id.name}) üretilecek/hazırlanacak.{task_description_suffix}"),
        ...
        return {
            ...
            'params': {
                ...
                'message': _("İlgili projelerde ürünler için görevler başarıyla oluşturuldu."),
                'type': 'success',
                'sticky': False,
            }
        }
